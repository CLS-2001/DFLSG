...
class ICFGPEDES():
    ...
    def __init__(self, root='', verbose=True):
        ...
        self.train, self.train_id_container, self.train_container = self._process_anno(self.train_annos, training=True)
        self.test, self.test_id_container = self._process_anno(self.test_annos)
        self.val, self.val_id_container = self._process_anno(self.val_annos)

        if verbose:
            logger.info("=> ICFG-PEDES Images and Captions are loaded")
    ...
    def _process_anno(self, annos: List[dict], training=False):
        ...
        if training:
            ...
            for anno in annos:
                ...
                img_paths_c.append(img_path)
                image_pids_c.append(pid)
                captions = anno['captions'] # caption list
                for caption in captions:
                    dataset.append((pid, image_id, img_path, caption))
                    captions_c.append(caption)
                    caption_pids_c.append(pid)
                image_id += 1
            ...
            container = {
                "image_pids": image_pids_c,
                "img_paths": img_paths_c,
                "caption_pids": caption_pids_c,
                "captions": captions_c
            }

            return dataset, pid_container,container
        else:
            dataset = {}
            img_paths = []
            captions = []
