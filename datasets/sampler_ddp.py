...
def No_index(a, b): #筛选a中不等于的元素形成一个新的列表
...
class RandomMultipleGallerySampler(Sampler):
    def __init__(self, data_source, num_instances=4):
        ...
        for index, (_, pid) in enumerate(data_source):
            if pid < 0:
                continue
            self.index_pid[index] = pid  #所有的实例对应的伪标签
            self.pid_index[pid].append(index) #每个簇中的实例对应的索引位置

        self.pids = list(self.pid_index.keys()) #光把簇的伪标签拿出来放到pids列表中
        self.num_samples = len(self.pids) #簇的数量

    def __len__(self):
        return self.num_samples * self.num_instances

    def __iter__(self):
        indices = torch.randperm(len(self.pids)).tolist() #生成一个随机排列的索引列表，其长度等于self.pids列表的长度
        ret = []#放的随机选择的簇的四个实例的索引，四个为一组，优先选择不同相机拍摄的索引，其次选择同一相机拍摄的实例

        for kid in indices:#随机生成一个大小为簇的数量的随机索引，用kid对这个随机索引进行遍历
            i = random.choice(self.pid_index[self.pids[kid]])  #从随机挑选的簇id中再随机取出一个实例的索引

            _, i_pid = self.data_source[i] #提取出实例的伪标签，和相机id

            ret.append(i) #把索引存入ret中

            pid_i = self.index_pid[i]
            index = self.pid_index[pid_i] #拿出簇id是pid_i的所有实例
            select_indexes = No_index(index, i)
            if not select_indexes:
                continue
            if len(select_indexes) >= self.num_instances: #如果其他的实例数量大于4
                ind_indexes = np.random.choice(select_indexes, size=self.num_instances-1, replace=False)#再随机选三个实例的索引
            else:
